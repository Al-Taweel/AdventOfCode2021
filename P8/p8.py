        ...
        for row in board:
            for number in range(len(row)):
                if row[number] == n:
                    ...
                    if row[number] == '0':
                        row[number] = '-1'  # cater for the special case of 0

# Check that all boarda have been solved.
...
def check_winning_board(boards):
    for board in boards:
        if board[0][0] == '-1000': # already bingo
            continue
        #check rows
        for row in board:
            bingo = True
            ...
            if bingo:
                temp = board[0][0]
                board[0][0] = '-1000' # Mark board as solved
                if last_board(boards):
                    board[0][0] = temp  # if last board, we need to remove the marker for correct calculations
                    return board
        # check columns
